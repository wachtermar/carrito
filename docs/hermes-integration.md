...
- Hermes prompt simulations: invoke `/carrito-shopping` or `--skills carrito-shopping` with shopping, weekly meal plan, imported recipes, diet, nutrition/macros, pantry, waste-reduction, cart-review, and guarded cart-prep requests. Verify agents report `artifact_audit.hermes_trust_summary`, `safe_to_build`, `safe_to_cook`, `safe_to_use_recipes`, `safe_to_report_nutrition`, `may_present_recipes_as_cookable`, and `may_present_pdf_as_complete` separately, and treat audit exit 30 as an untrusted bundle rather than a readiness-only diagnostic.
