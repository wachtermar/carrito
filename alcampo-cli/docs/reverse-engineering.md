...
The password is accepted only from a no-echo terminal prompt, `ALCAMPO_PASSWORD`, or `--password-stdin`; it is never stored. The resulting Alcampo cookies, homepage CSRF token, customer id, visitor id, delivery destination, and market hints are saved into the same session config as imported sessions. The CLI does not bypass CAPTCHA, MFA, disabled-account, consent, or risk controls.
