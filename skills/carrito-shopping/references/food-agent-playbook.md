...
4. Prefer the one-command path for low-intervention runs after intake:
   ...
   - Use `--adult-servings`, `--child-servings`, and `--toddler-servings`, or `--household-profile household.json`, instead of `--servings` when the household is mixed or meal participation differs by person. Use household `default_meal_participation` rules with `days`, `from_day`, or `to_day` when participation changes across the week.
   ...
   - For shareable cooking output, rely on `food run` automatic HTML generation and pass `--html-out output/html/<descriptive-name>.html` for a stable path. Use `carrito food html run.json --out output/html/<descriptive-name>.html [--cover-image path-or-url]` only to regenerate an existing run. Use the HTML page as the primary user-facing artifact on mobile; keep `--pdf-out`/`food pdf` as print or evidence fallback.
...
5. Present review output before cart mutation:
   ...
   - the mobile HTML cooking page path from `food run` (`--html-out` or the automatic path next to `--run-out`/`--pdf-out`)
