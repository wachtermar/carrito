...
4. Prefer the one-command path for low-intervention runs after intake:
   ...
   - If this exits 20, the run still produced diagnostic artifacts. Parse `run.json`, `readiness.json`, `recipe_intake.json`, `recipe_quality.json`, `serving_plan.json`, `scaled_mealplan.json`, `nutrition_ledger.json`, `budget_repair.json`, `budget_deal.json`, `pantry.json`, and `audit.json`; report `safe_to_build`, `safe_to_cook`, `safe_to_use_recipes`, `safe_to_report_nutrition`, `budget_repair_plan.status`, `safe_to_report_budget`, and `safe_to_report_deals` separately. Do not use `basket.txt` for cart preparation when `safe_to_build` is false, do not call the meal plan cook-ready when `safe_to_cook` or `safe_to_use_recipes` is false, do not call calories/macros complete when `safe_to_report_nutrition` is false, and do not claim the plan is within budget when `safe_to_report_budget` is false.
