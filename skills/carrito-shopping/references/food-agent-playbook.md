...
4. Prefer the one-command path for low-intervention runs after intake:
   ...
   - If this exits 20, the run still produced diagnostic artifacts. Parse `run.json`, `readiness.json`, `recipe_intake.json`, `recipe_quality.json`, `serving_plan.json`, `scaled_mealplan.json`, `nutrition_ledger.json`, `pantry.json`, and `audit.json`; report `safe_to_build`, `safe_to_cook`, `safe_to_use_recipes`, and `safe_to_report_nutrition` separately. Do not use `basket.txt` for cart preparation when `safe_to_build` is false, do not call the meal plan cook-ready when `safe_to_cook` or `safe_to_use_recipes` is false, and do not call calories/macros complete when `safe_to_report_nutrition` is false.
...
5. Present review output before cart mutation:
   ...
   - `readiness_gate.status`, `readiness_gate.safe_to_build`, `readiness_gate.safe_to_cook`, `readiness_gate.safe_to_use_recipes`, `readiness_gate.safe_to_report_nutrition`, cook readiness status, recipe quality status, serving readiness status, nutrition readiness status, blocking issues, caveats, exit code, and remediation
   - `artifact_audit.status`, `artifact_audit.hermes_trust_summary.trustworthy`, `may_present_basket_as_ready`, `may_present_cook_ready`, `may_present_recipes_as_cookable`, `may_present_pdf_as_complete`, `may_present_nutrition_numbers`, and any required user warning
