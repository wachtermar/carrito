...
`food html` accepts recipe, meal plan, shop, or combined `food_run` JSON and writes a mobile-first cooking page with day navigation, recipe ingredients, cooking steps, shopping evidence, and readiness caveats. Use it for shareable/cookable user artifacts, especially on mobile. Generate it from the current run JSON into a stable path such as `output/html/<descriptive-name>.html`; use `--cover-image` only with a real dish/recipe image for the first active recipe. Product package images must stay in the shopping section as Alcampo product evidence and must never be used as recipe or hero food photos. If a recipe image is missing, the HTML page says so instead of substituting a package photo.
