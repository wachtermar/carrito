...
carrito food recipes add --url https://example.com/recipe --json
...
carrito food html <recipe-or-plan-shop-run.json> --out food-plan.html [--cover-image path-or-url]
...
For Hermes request-satisfaction runs, add `--intent-file intent.json --intent-out intent.json --constraint-report-out constraint_report.json --require-intent-ready`. Convenience flags `--max-cook-minutes`, `--exclude-ingredient`, `--exclude-allergen`, and `--diet-rule` merge hard constraints into the loaded intent.

`food plan` and `food run` require a people count from either `--people` or `profile.json`; they intentionally fail instead of inventing a household size. `food run` is the preferred low-intervention path after intake: it loads profile and pantry memory, generates the meal plan, applies optional structured recipe intake from `--recipe-file`, `--recipe-dir`, or `--recipe-url`, applies optional serving scaling from `--servings`, `--adult-servings`, `--child-servings`, `--toddler-servings`, or `--household-profile`, subtracts pantry/fridge items from the scaled requirements, applies structured pantry resolution when requested, shops the remaining ingredients, optionally enriches selected products from Alcampo detail pages, builds a quantity ledger, can attempt product-level budget repair with `--repair-budget`, builds final selected-SKU product evidence with `--product-evidence-out --refresh-product-evidence`, builds a nutrition ledger, builds a budget/deal report, attempts audited missing-product recovery with `--recover-missing`, can repair blocked generated meal plans with `--allow-recipe-swap`, can run audited deal-aware product re-selection with `--optimize-basket`, writes a combined `food_run` JSON artifact with `--run-out`, can write `recipe_intake_plan` with `--recipe-intake-out`, can write `recipe_quality_report` with `--recipe-quality-out`, can write `budget_repair_plan` with `--budget-repair-out`, can write `budget_deal_report` with `--budget-deal-out`, can write `serving_plan` with `--serving-plan-out`, can write `scaled_mealplan` with `--scaled-mealplan-out`, can write a standalone `quantity_ledger` with `--quantity-ledger-out`, can write a standalone `product_evidence_report` with `--product-evidence-out`, can write a standalone `nutrition_ledger` with `--nutrition-ledger-out`, can write `recovery_plan` with `--recovery-out`, can write `recipe_swap_plan` with `--recipe-swap-out`, can write `basket_optimization_plan` with `--basket-optimization-out`, can write `pantry_resolution` with `--pantry-out`, can write planned pantry consumption with `--pantry-consumption-out`, writes the final `readiness_gate` with `--readiness-out`, optionally renders a complete meal-plan-and-shopping PDF with `--pdf-out`, and returns one JSON object. For full meal-planning requests, prefer `--servings <n> --run-out --serving-plan-out --scaled-mealplan-out --quantity-ledger-out --product-evidence-out --refresh-product-evidence --nutrition-ledger-out --budget-repair-out --budget-deal-out --recipe-intake-out --recipe-quality-out --recovery-out --recipe-swap-out --basket-optimization-out --pantry-out --pantry-consumption-out --readiness-out --pdf-out --enrich-products --strict-quantity --strict-servings --strict-recipe-quality --recover-missing --allow-recipe-swap --optimize-basket --repair-budget --basket-objective safe-balanced --deal-aware --default-pantry minimal-spanish --allow-assumed-pantry --require-safe-basket --require-cook-ready` over creating a shop-only PDF; add `--require-fresh-product-evidence --no-product-evidence-cache` when current live product/price claims are required; use adult/child/toddler flags or a household profile instead of `--servings` for mixed households. Saved `food pantry` memory is merged into structured pantry resolution as confirmed evidence, and `--pantry-profile pantry_profile.json` can add normalized pantry items with `value`, `unit`, `base_value`, and `base_unit`. With `--require-safe-basket`, `--require-cook-ready`, `--require-nutrition-ready`, `--require-budget-ready`, or `--require-fresh-product-evidence`, exit code 20 means the run completed and wrote diagnostic artifacts; parse `readiness_gate.blocking_issues`, `readiness_gate.safe_to_build`, `readiness_gate.safe_to_cook`, `readiness_gate.safe_to_use_recipes`, `readiness_gate.safe_to_use_product_evidence`, `readiness_gate.product_evidence_status`, `readiness_gate.safe_to_report_nutrition`, `readiness_gate.safe_to_report_budget`, `readiness_gate.safe_to_report_deals`, `recipe_quality_report`, `product_evidence_report`, `serving_plan`, `scaled_mealplan`, `nutrition_ledger`, `budget_repair_plan`, `budget_deal_report`, and `pantry_resolution`, then report basket, cooking, recipe, product evidence, nutrition, and budget/deal readiness separately. Do not use the basket file for cart mutation when `safe_to_build` is false. When `safe_to_build` is true but `safe_to_cook` is false, the basket lines are structurally valid but the meal plan still needs pantry, serving, recipe-quality, or ingredient confirmation. When `safe_to_use_product_evidence` is false, do not call selected products, prices, offers, stock, or product-label nutrition current. When `safe_to_report_nutrition` is false, calories and macros can be shown only with explicit coverage caveats. When `safe_to_report_budget` is false, do not claim the plan is within budget. Say budget repair succeeded only when `budget_repair_plan.status` is `attempted_applied`, final budget readiness is reportable, and audit trust permits budget claims; failed repair is diagnostic and must not be replaced with unvalidated manual substitutions.

`--intent-file intent.json` loads a structured `meal_run_intent` built by the agent, not by free-form parsing in Go. Use hard intent fields only for explicit user constraints or saved profile facts. Vague terms must be mapped before they become hard claims: "cheap" to a budget or price rule, "healthy" to nutrition/diet targets, and "quick" to a cook-time limit. `--intent-out` writes the normalized intent into the bundle, and `--constraint-report-out` writes deterministic satisfied, not-satisfied, unknown, and not-applicable checks. The report evaluates plan coverage, serving units, budget, nutrition targets, ingredient/allergen exclusions, diet rules, cooking time, pantry requirements, and recipe/PDF completeness using artifact evidence. Unknown hard constraints fail the claim by default; missing nutrition labels are unknown evidence, not zeros. Deterministic ingredient/allergen checks are string-evidence checks over recipe/product data and are not a medical allergy guarantee. `--require-intent-ready` exits 20 when the request cannot be safely claimed as satisfied, but this does not by itself make a structurally safe basket unusable.

Recipe intake behavior: `--recipe-file` and `--recipe-dir` load structured Carrito recipe JSON without requiring it to pass the saved-library validator first; incomplete recipes become quality warnings or blockers in `recipe_quality_report` instead of disappearing. `--recipe-url` imports only schema.org Recipe JSON-LD and never scrapes arbitrary page text. Imported recipes replace active meal slots deterministically in order and cycle if fewer imported recipes are provided than planned slots. The quality report checks stable ids/titles, source evidence, base servings, cooking steps, vague required ingredients such as `meat` or `verduras`, parseable required quantities, and recipe image provenance. `--strict-recipe-quality` makes quality blockers affect `readiness_gate.safe_to_use_recipes` and cook readiness; `--require-recipe-source` and `--require-recipe-images` enforce provenance and cached/local images. Missing images are non-blocking by default; broken images are recorded as quality caveats. The PDF includes a `Recipe sources and quality:` section, and the basket comment header includes recipe quality status when quality was evaluated.

For Hermes-grade runs, also write `--manifest-out manifest.json --audit-out audit.json --audit-mode fail`. The manifest records artifact paths, hashes, sizes, readiness summary, generation exit code, and fingerprints, including recipe-set, recipe-quality, recipe-image, product-evidence, budget-repair, budget-deal, intent, and constraint-satisfaction fingerprints. The audit reopens the written run JSON, sidecars, guarded basket, and PDF from disk; verifies hashes, sidecar equality, fingerprint consistency, basket actionability, PDF trust markers including final Alcampo product evidence, request-constraint markers, and basic ledger/product-evidence/nutrition/recipe-quality/budget-repair/budget-deal/constraint consistency; and returns exit code 30 when the bundle is contradictory or unsafe to trust. Exit 30 overrides readiness claims: regenerate the artifacts before saying the basket, cooking status, PDF, current products/prices, recipe quality, nutrition totals, budget repair, budget/deal status, or request satisfaction are ready. `food validate-run` runs the same audit later for an existing bundle, and agents should read `artifact_audit.hermes_trust_summary.trustworthy`, `may_present_basket_as_ready`, `may_present_cook_ready`, `may_present_recipes_as_cookable`, `may_present_pdf_as_complete`, `may_present_nutrition_numbers`, `may_present_budget_as_ready`, `may_present_deals_as_ready`, `may_present_request_as_satisfied`, `may_present_alcampo_products_as_current`, `may_present_product_prices_as_current`, and `may_present_product_nutrition_as_current` before making user-facing claims.
...
Serving behavior: serving scaling happens before pantry subtraction, shopping, recovery, recipe swaps, basket optimization, and PDF rendering. `--servings N` sets an explicit target for every planned recipe slot; `--adult-servings`, `--child-servings`, and `--toddler-servings` calculate weighted targets for mixed households; `--household-profile household.json` can specify members plus per-meal and per-day participation. `default_meal_participation` rules accept `meal_slot`, `member_ids`, `days`, `from_day`, and `to_day`, so a week can include different weekday and weekend participants without averaging serving counts. `--allow-leftovers` or `--leftover-servings N` can cook extra servings intentionally; `--no-leftovers` keeps cooked serving units equal to target serving units. `--strict-servings --require-cook-ready` blocks cooking readiness when a recipe cannot be scaled safely, while `--no-serving-scaling` preserves legacy recipe quantities. The `serving_plan` artifact records target/cooked serving units, participants, leftovers, warnings, and fingerprints; `scaled_mealplan` records original and scaled ingredient quantities, scale factors, rounded-piece notes, and blocked/unparseable quantities.

Recipe library behavior: embedded seed recipes are always available, and the editable library lives in the SQLite database at `CARRITO_CONFIG_DIR/food/recipes.db` or `~/.carrito/food/recipes.db`. The database stores normalized recipe, tag, ingredient, equipment, step, substitution, allergen-note, nutrition, source URL, and full-text search tables. User recipes added through `food recipes add` override seed recipes with the same `id`. Legacy `food/recipes/*.json` files are imported once into SQLite for migration, but new recipes should be added through the CLI. `food recipes add --url <recipe-page-url> --json` imports schema.org Recipe JSON-LD webpages into the saved recipe library and preserves `source_url`; it does not scrape arbitrary unstructured page text. `food recipes add --from-text` parses comma- or newline-separated ingredient lists and auto-derives Spanish search terms. `food recipes search <query> --profile` applies saved profile diets, allergies, and dislikes.
...
Fresh full `food run` bundles automatically write a mobile-first cooking page next to `--run-out` or `--pdf-out`; pass `--html-out output/html/<descriptive-name>.html` for a stable user-facing path, or `--no-html` for diagnostics-only scripts. `food html` still accepts recipe, meal plan, shop, or combined `food_run` JSON when you need to regenerate an existing artifact. The page includes day navigation, recipe ingredients, cooking steps, shopping evidence, and readiness caveats. Use it for shareable/cookable user artifacts, especially on mobile. Use `--cover-image` only with a real dish/recipe image for the first active recipe. Product package images must stay in the shopping section as Alcampo product evidence and must never be used as recipe or hero food photos. If a recipe image is missing, the HTML page says so instead of substituting a package photo.

`food pdf` accepts recipe, meal plan, shop, or combined `food_run` JSON. Treat PDFs as print/evidence artifacts rather than the primary shareable cooking experience. When a user asks for a shareable cooking PDF from an existing run, regenerate it from the run JSON into a stable path such as `output/pdf/<descriptive-name>.pdf` instead of copying an old `/tmp` PDF. Combined run PDFs include the day-by-day meal plan, recipe source/quality/image provenance, serving assumptions, scaled ingredients, recipe ingredients and cooking instructions, selected products, product images, package math, quantity-ledger confidence, budget repair actions, budget/deal evidence, nutrition ledger evidence, readiness verdict, blocking issues or caveats, estimated totals, basket lines, consumed nutrition totals when coverage supports them, recipe-declared nutrition as a separate caveat, and cart/cooking/nutrition/budget-safety caveats. The PDF renderer embeds retrievable recipe cover, step, and product images from local paths, data image URLs, or HTTP(S) image URLs. When `recipe_quality_report` is present, prefer cached/local recipe image paths produced by recipe intake; if an image cannot be loaded, the PDF falls back to the original image URL text instead of failing the whole document.
